...
def processHand(myList):
    ...
    for val in myList:
        ...
        if dir == "R":
            xList.append((x, y, x + intVal, y, 0, sumLength))
            x += intVal
        elif dir == "L":
            xList.append((x - intVal, y, x, y, 1, sumLength))
            x -= intVal
        elif dir == "U":
            yList.append((x, y, x, y + intVal, 0, sumLength))
            y += intVal
        elif dir == "D":
            yList.append((x, y - intVal, x, y, 1, sumLength))
            y -= intVal
        else:
...
def findClosestIntersection(line, listOfLines):
    MIN = 100000
    for (x1, y1, x2, y2, dir, lineLength) in listOfLines:
        (nx1, ny1, nx2, ny2, ndir, nlength) = line
        if nx1 == nx2:  #line is vertical
            if y1 != y2:
                ...
                if nx1 != 0 and y1 != 0:
                    #compute the length for both arms
                    val = lineLength + nlength
                    if dir > 0:
                        val += x2 - nx2
                    else:
                        val += nx2 - x1
